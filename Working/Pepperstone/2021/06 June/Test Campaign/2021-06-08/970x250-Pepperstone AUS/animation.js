...
gsap.set("#headline-wrapper, #subheadline-wrapper, #subheadline2-wrapper, #legal-wrapper, #cta-container", {rotationZ:0.01, force3D:false});
...
function startAnimation() {  
    tl = gsap.timeline({
        onStart: function(){
            ...
            if(Adlib.isEmpty(defaultValues.legal) && Adlib.isEmpty(defaultValues.legal2)) {
                document.getElementById("legal-wrapper").style.display = "none";
                document.getElementById("header-container").style.paddingBottom = "16px";
            }
            /*else if(defaultValues.trigger == "noPanel" && (Adlib.isEmpty(defaultValues.legal) || Adlib.isEmpty(defaultValues.legal2))) {
                document.getElementById("header-container").style.width = "100%";
            }*/
            if(Adlib.isEmpty(defaultValues.frame1Subheadline) && (!Adlib.isEmpty(defaultValues.legal) || !Adlib.isEmpty(defaultValues.legal2))) {
    ...
    tl.to("#mainContent", {duration: 0, visibility: "visible"})
      .from("#headline-wrapper, #subheadline-wrapper", {duration: 0.5, y: "20px", opacity: 0, force3D: false},'+0.5')
      .from("#cta-container", {duration: 0.5, x: "-20%", opacity: 0},'-=0.5')
      ...
      .to("#headline1, #subheadline1", {duration: 0.5, opacity: 0, y:"10px"},'+=2')
      .from("#headline2, #subheadline2", {duration: 0.5, opacity: 0, y:"10px", onComplete: function(){
          //NO HEADLINE 3
          var action = (Adlib.isEmpty(defaultValues.frame3Headline)) ? "end" : "screenshot";
          skipTextAnimation(action);
      }})
      
      .to("#headline2, #subheadline2", {duration: 0.5, opacity: 0, y:"10px"},'+=2')
      .from("#headline3, #subheadline3", {duration: 0.5, opacity: 0, y:"10px"});
}
