...
function textAnimation(_h, _cta){
    ...
    gsap.to([_h, _h.parentNode], 0, {opacity:1});
    gsap.fromTo([_cta], 0.3, {y:-20}, {y:0, opacity:1, ease:"power1.out", delay:0.2*copy.length});
...
function frame2Anim() {
    ...
    gsap.delayedCall(2.5, callAdlibScreenshot);
    gsap.delayedCall(3, stopAdlibScreenshot);
}
