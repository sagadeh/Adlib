...
function startAnimation() {  
	...
	if(discHeight > 17){
		gsap.set(["#headline1-bg", "#headline2-bg", "#headline3-bg", "#extralegal-bg", "#extralegal-wrapper"], { height: bgHeight});
		gsap.set("#ctaDiv", { bottom: "0"});
		gsap.set("#endframeContainer", { bottom: "4px"});
		gsap.set("#ctaboilerContainer", { bottom: ctaBottom});
	} else {
		gsap.set("#ctaboilerContainer", { bottom: ctaBottom2});
	}
...
function frame3Animation(){
	...
	gsap.to("#bgimage-wrapper2", {duration: .3, ease: "power1.Out", opacity: 1, dekay: .5});
	gsap.to("#bgImage2", {duration: 12, ease: "power1.inOut", scale: 1.1, rotationZ: 0.01});
}
