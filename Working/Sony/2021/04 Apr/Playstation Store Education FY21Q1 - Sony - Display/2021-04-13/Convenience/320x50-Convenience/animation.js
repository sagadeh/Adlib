...
function startAnimation() {  
    ...
    tl.set(["#bg_img", "#room", "#figure", "#laser"], { scale:.5, left:'100%', top:'50%', xPercent:'-50', yPercent:'-50', transformOrigin:"50% 50%", force3D:true, rotation: .001 }, "+=0");

    tl.set(["#bg_img"], { scale: 1, left:'50%', top:'50%', xPercent:'-50', yPercent:'-50'}, "+=0");

    tl.set(["#room"], { scale: 0, x:0, y:0,transformOrigin:"0% 50%"});

    tl.set(["#laser"], { x:-120, y:-40, transformOrigin:"0% 50%"});

    tl.set(["#figure"], { x:-150, y:-60, transformOrigin:"0% 50%"}, "+=0");
    ...
    tl.to(["#room"], .75, { scale: .5,x:0, y:0, rotation: 0, ease: Power1.easeOut}, .5);

    tl.to(["#figure"], .75, { x:0, y:0, ease: Power1.easeOut}, .75);

    tl.to(["#laser"], .75, { x:0, y:0, rotation: 0, ease: Power1.easeOut}, 1);

    tl.to(["#grad"], .75, { x:0, ease: Power1.easeOut}, 1);

    tl.to(["#t1"], .5, { autoAlpha: 1, y:0, ease: Power1.easeOut ,onComplete:takeScreenshot}, 1.7);

    tl.to(["#t1"], .5, { autoAlpha: 0, y:10, ease: Power1.easeOut }, 4);

    tl.to(["#t2"], .5, { autoAlpha: 1, y:0, ease: Power1.easeOut ,onComplete:takeScreenshot}, 4.5);

    tl.to(["#blue_bg"], .75, { x:0, ease: Power1.easeOut}, 7);

    tl.to(["#t3"], .5, { autoAlpha: 1, y:0, ease: Power1.easeOut ,onComplete:animationEnd}, 7.6);

    //tl.to(["#cta"], .5, { autoAlpha: 1, y:0, ease: Power1.easeOut }, 10.5);
}
